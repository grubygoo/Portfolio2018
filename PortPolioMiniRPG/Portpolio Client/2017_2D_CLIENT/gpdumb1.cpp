...
if (!(bob->attr & BOB_ATTR_VISIBLE))
   return(1);
	...
	D3DXVECTOR3 pos = D3DXVECTOR3((bob->x - g_left_x) * static_cast<float>(TILE_WIDTH) + 10,
		(bob->y - g_top_y) * static_cast<float>(TILE_WIDTH) + 10, 0.0);
	...
	wsprintf(bob->message, L"%d", bob->ID);
	Draw_Text_D3D(bob->message,static_cast<int>(pos.x),static_cast<int>(pos.y),D3DCOLOR_ARGB(255,255,0,0));
