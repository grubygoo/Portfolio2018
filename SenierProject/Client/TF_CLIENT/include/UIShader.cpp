...
D3D12_SHADER_BYTECODE UIShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob, UINT nIndex)
{
	...
	if (nIndex == SANYANGSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "WARRIORSKILL_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == FRAME)
		return(CompileShaderFromFile(L"UI.hlsl", "FRAME_VS", "vs_5_1", ppd3dShaderBlob));
}
...
{
	//						개수
	m_pTexture = new CTexture(12, RESOURCE_TEXTURE2D, 0);
	...
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/cool_frame.dds", 8);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/cool_sword2.dds", 9);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/cool_wing2.dds", 10);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/cool_horn2.dds", 11);
...
void UIShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	for (int i = 0; i < m_nPipelineStates; i++)
	{
		...
		else if (i == 9 || i == 10 || i == 11)
		{
			...
			{
				if (m_ppd3dPipelineStates)
					pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[11]);

				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(11).m_d3dSrvGpuDescriptorHandle);
