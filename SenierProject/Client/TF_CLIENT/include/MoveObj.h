...
public:
	void  SetDirection(WORD direction) { std::unique_lock<shared_mutex> lock(MM[0]); m_Direction = direction; }
	void  SetAnimState(BYTE state)     { std::unique_lock<shared_mutex> lock(MM[1]); m_AnimState =  state; }
