...
void CScene::PlayerAttack(char * packet)
{
	...
	/*이부분에서 저 아이디의 벡터원소가 모델이 요정이면 요정이펙트 생성하면 된다.*/
	sc_packet_attack_player* Packet = (sc_packet_attack_player*)packet;
	m_pPlayerVector[Packet->id]->SetAnimState(ATTACKSTATE);

}

void CScene::PlayerSkill(char * packet)
{

	/*이부분에서 다른사람이 스킬 썼다는 패킷임*/
	/*이부분도 모델 이 뭔지에따라 위치에 스킬이펙트 생성해주면됨*/
	sc_packet_other_skill* Packet = (sc_packet_other_skill*)packet;
	WORD id = Packet->id;
	m_pPlayerVector[id]->IsSkill = true;
}

void CScene::PlayerTeleport(char * packet)
{

	/*다른사람 텔레포트 썻다는 부분 현재위치에다가 이펙트 생성하면됨*/
	sc_packet_other_teleport*  Packet = (sc_packet_other_teleport*)packet;
	//m_pPlayerVector[Packet->id]->IsTell = true;
	m_pPlayerVector[Packet->id]->SetTel(true);
...
void CScene::RespawnPlayer(char * packet)
{
	sc_packet_respawn_player* Packet = (sc_packet_respawn_player*)packet;
	m_pPlayer->SetActive(true);
	m_pPlayer->SetWPosition(Packet->pos);
	m_pPlayer->SetHp(Packet->hp);
